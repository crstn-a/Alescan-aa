    ...
    <div style={{
      ...
      <header style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div>
            <p style={{ fontSize: 14, fontWeight: 700, color: '#fff', margin: 0 }}>Alescan</p>
            <p style={{ fontSize: 11, color: 'rgba(255,255,255,.45)', margin: 0 }}>SRP Scanner</p>
          </div>
        </div>
        <div style={{
      ...
      <div style={{ flex: 1, position: 'relative', overflow: 'hidden', minHeight: 0 }}>
        ...
        {cameraState === 'error' && (
          <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 16, padding: 32, background: 'rgba(0,0,0,.7)' }}>
            <div style={{ width: 56, height: 56, borderRadius: 14, background: '#fef2f2', display: 'flex', alignItems: 'center', justifyContent: 'center', color: C.error }}>
            ...
            <div style={{ textAlign: 'center' }}>
              <p style={{ fontSize: 15, fontWeight: 700, color: '#fff', marginBottom: 6 }}>Camera unavailable</p>
              <p style={{ fontSize: 13, color: 'rgba(255,255,255,.6)', lineHeight: 1.6, marginBottom: 16 }}>{cameraError}</p>
            ...
            <div style={{ position: 'relative', width: 'min(75vw, 75vh, 280px)', aspectRatio: '1/1' }}>
              ...
              {scanning && (
                <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <div style={{ width: 44, height: 44, border: `3px solid rgba(255,255,255,.15)`, borderTopColor: C.primary, borderRadius: '50%', animation: 'spin .75s linear infinite' }} />
                </div>
              )}
            </div>
          </div>
        ...
        {isReady && !scanning && (
          <div style={{ position: 'absolute', bottom: 'calc(50% - min(37.5vw, 37.5vh, 140px))', left: 0, right: 0, display: 'flex', justifyContent: 'center', pointerEvents: 'none' }}>
            <div style={{ background: 'rgba(0,0,0,.5)', backdropFilter: 'blur(8px)', borderRadius: 20, padding: '6px 16px', border: '1px solid rgba(255,255,255,.12)' }}>
              <p style={{ fontSize: 12, color: 'rgba(255,255,255,.75)', margin: 0, fontWeight: 500 }}>Point at the commodity and tap Scan</p>
            </div>
          </div>
        )}
      </div>
      ...
      <div style={{
        ...
        {feedback && (
          <div style={{
            ...
            background: feedback.type === 'warn' ? 'rgba(245,158,11,.25)' : 'rgba(239,68,68,.25)',
            border: `1px solid ${feedback.type === 'warn' ? 'rgba(251,191,36,.3)' : 'rgba(248,113,113,.3)'}`,
            ...
            <p style={{ fontSize: 13, color: 'rgba(255,255,255,.85)', margin: 0, fontWeight: 500, lineHeight: 1.4 }}>{feedback.text}</p>
          </div>
        )}
        ...
        <div className="scan-btn-wrap" style={{ position: 'relative', cursor: isReady && !scanning ? 'pointer' : 'default' }} onClick={handleScan}>
          ...
          <div className="scan-btn-inner" style={{
            ...
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            boxShadow: scanning ? 'none' : '0 0 32px rgba(34,197,94,.5)',
        ...
        <p style={{ fontSize: 12, color: 'rgba(255,255,255,.45)', margin: 0, fontWeight: 500 }}>
          {scanning ? 'Identifying commodity...' : isReady ? 'Tap to scan' : 'Starting camera...'}
        </p>

        {/* Commodity chips section removed as per request */}
      </div>
    </div>
  )
}